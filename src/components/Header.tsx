...
export default function Header() {
  ...
  return (
    <header className="flex items-center justify-between border-b border-white/10 h-14 px-3 sm:px-9 transition">
      <Logo />
      <nav className="h-full">
        
        <ul className="flex gap-x-6 text-sm h-full transition">
          {routes.map((route) => (
            <li
              key={route.path}
              className={cn(
